...
pub fn eval<U>(expression: Expression, ctx: &mut Context<U>, caller: &str) -> Expression
where
    U: Send + 'static,
{
    match expression {
        Expression::Deferred(src) => {
            ...
            eval(expr, ctx, caller)
        }
        Expression::Ident(ident) => eval(ctx.variables.get(&ident), ctx, caller),
        Expression::Str(_) | Expression::Int(_) | Expression::Float(_) | Expression::Array(_) => expression,
        Expression::Binary { lhs, rhs, op } => eval_op(*lhs, *rhs, op, ctx, caller),
        ...
        Expression::Fn { fun, args } => match *fun {
            Expression::Ident(key) => {
                let args = args.into_iter().map(|arg| eval(arg, ctx, caller)).collect();
                ctx.call(&key, args, caller)
            }
            _ => Expression::Null,
        },
        Expression::Declaration { ident, value } => {
            let value = eval(*value, ctx, caller);
        ...
        Expression::Assignment { mut lhs, rhs } => {
            let rhs = eval(*rhs, ctx, caller);

            if let Some(lhs) = assign_eval_mut(&mut lhs, ctx, caller) {
                *lhs = rhs;
            }
            Expression::Null
        }
        Expression::Index { src, key } => eval_index(*src, *key, ctx, caller),
        Expression::Null => Expression::Null,
    }
}

fn assign_eval_mut<'a, U>(expr: &mut Expression, ctx: &'a mut Context<U>, caller: &str) -> Option<&'a mut Expression>
where
    U: Send + 'static,
{
    match expr {
        ...
        Expression::Index { src, key } => {
            let Expression::Int(key) = eval(*key.clone(), ctx, caller) else { return None };
            let src = assign_eval_mut(src, ctx, caller)?;
            match src {
...
fn eval_index<U>(src: Expression, key: Expression, ctx: &mut Context<U>, caller: &str) -> Expression
where
    U: Send + 'static,
{
    let src = eval(src, ctx, caller);
    match src {
        Expression::Array(mut expressions) if !expressions.is_empty() => {
            let Expression::Int(key) = eval(key, ctx, caller) else { return Expression::Null };
            if key as usize >= expressions.len() {
                return Expression::Null;
            }

            let expression = expressions.remove(key as usize);
            eval(expression, ctx, caller)
        }
        ...
        Expression::Index { .. } => {
            let src = eval(src, ctx, caller);
            eval_index(src, key, ctx, caller)
        }
    }
}

fn eval_op<U>(lhs: Expression, rhs: Expression, op: Op, ctx: &mut Context<U>, caller: &str) -> Expression
where
    U: Send + 'static,
{
    match (eval(lhs, ctx, caller), eval(rhs, ctx, caller)) {
